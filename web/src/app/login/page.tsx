...
export default function LoginPage() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        router.push("/dashboard");  // returning user → dashboard
      } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-background via-muted/30 to-background p-4 relative overflow-hidden">
      ...
      >
        <Card className="bg-linear-to-br from-card/80 to-card/40 backdrop-blur-xl border border-primary/20 shadow-[0_20px_60px_-20px_rgba(30,64,175,0.4)]">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-center">
              {isLogin ? "Enter your credentials to access your civic dashboard" : "Sign up to start your election journey"}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-background/50 px-2 text-muted-foreground backdrop-blur-md">
                  Or continue with
                </span>
              </div>
            </div>

            <Button variant="outline" type="button" className="w-full bg-background/50 border-white/10" onClick={handleGoogleAuth} disabled={loading}>
